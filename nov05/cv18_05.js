...
function draw() {
  ...
  if (drawCount++ > maxDrawing) noLoop()
  	stroke(random(100), 40, 40)
	noFill()
	let r = 5 + random(90)
  	if (random(100) < 70) ellipse(random(width), random(height), r, r)
  	for (let i = 0; i < 20; i++){
...
Boid.prototype.update = function() {
  ...
  if (this.tail.length > this.tailCount) this.tail.splice(-1, 1)
  this.tail.push(createVector(this.position.x, this.position.y))
